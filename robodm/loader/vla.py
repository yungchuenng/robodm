...
class SliceConfig:
    ...
    min_slice_length: Optional[int] = None  # Minimum slice length (defaults to slice_length)
...
class RayVLALoader(BaseLoader):
    ...
    def __init__(
        ...
        logger.info(f"Initialized RayVLALoader with {len(self.file_paths)} files in {mode.value} mode")
    ...
    def _extract_slices(self, item) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            min_length = self.slice_config.min_slice_length or self.slice_config.slice_length
            
            if traj_length < min_length:
                logger.warning(f"Trajectory {file_path} too short ({traj_length} < {min_length})")
                return []
            ...
            if self.slice_config.random_start:
                ...
                start_positions = [random.randint(0, max_start) for _ in range(num_slices)]
            else:
                # Sequential slicing
            ...
            for start_idx in start_positions:
                end_idx = min(start_idx + self.slice_config.slice_length, traj_length)
                ...
                for key, values in full_data.items():
                    if isinstance(values, np.ndarray):
                        slice_data[key] = values[start_idx:end_idx:self.slice_config.stride]
    ...
    def split(self, *fractions: float, shuffle: bool = True):
        ...
        if sum(fractions) > 1.0:
            raise ValueError(f"Sum of fractions {sum(fractions)} must be <= 1.0")
        
        # Ray Dataset.split() doesn't support shuffle parameter
        # If shuffle is requested, shuffle the dataset first
        dataset_to_split = self.dataset.random_shuffle() if shuffle else self.dataset
        
        if len(fractions) == 1:
            # For single fraction, convert to train/test split
            return dataset_to_split.train_test_split(test_size=fractions[0], shuffle=False)
            ...
            return dataset_to_split.train_test_split(test_size=fractions[1], shuffle=False)
        else:
    ...
    def sample(self, num_samples: int, replace: bool = False):
        ...
        if not replace:
            ...
            import warnings
            warnings.warn("Sampling with replacement may not return exact count due to Ray API limitations")
...
class NonShuffleVLALoader(RayVLALoader):
    ...
    def __init__(self, path: Text, batch_size=1, num_workers=1, return_type="numpy"):
        logger.warning("NonShuffleVLALoader is deprecated. Use RayVLALoader instead.")
        super().__init__(
            path=path,
            mode=LoadingMode.TRAJECTORY,
            batch_size=batch_size,
