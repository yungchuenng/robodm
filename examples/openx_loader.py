...
def process_data(data_traj, dataset_name, index, destination_dir, video_codec):
    try:
        ...
        fog_x.Trajectory.from_list_of_dicts(
            data_traj,
            path=f"{destination_dir}/{dataset_name}/output_{index}.vla",
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert OpenX datasets to VLA format")
    ...
    parser.add_argument("--destination_dir",
                        type=str,
                        help="Destination directory for VLA files")
    parser.add_argument(
        "--max_episodes",
        type=int,
        default=None,
    ...
    # Load dataset
    loader = RLDSLoader(dataset_name=args.dataset_name,
    ...
    print(
        f"Conversion complete: {success_count}/{total_count} trajectories processed successfully"
    )
