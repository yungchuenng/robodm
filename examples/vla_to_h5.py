...
def write_to_h5(trajectory, dataset_name, index, destination_dir):
    ...
    try:
        with h5py.File(f"{destination_dir}/{dataset_name}/output_{index}.h5",
                       "w") as f:
...
def main():
    ...
    parser.add_argument("--data_dir",
                        required=True,
                        help="Path to the VLA data directory")
    ...
    parser.add_argument(
        "--destination_dir",
        required=True,
        help="Destination directory for output HDF5 files",
    )
    ...
    os.makedirs(os.path.join(args.destination_dir, args.dataset_name),
                exist_ok=True)
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        retry_queue = []
        try:
            for index, trajectory in tqdm(enumerate(loader),
    ...
    if retry_queue:
        ...
        with ProcessPoolExecutor(
                max_workers=args.max_workers) as retry_executor:
            retry_futures = []
