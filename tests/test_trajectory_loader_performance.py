...
class TestTrajectoryLoaderPerformance:
    ...
    def test_seeking_performance_benefit(self, large_trajectory_path):
        ...
        start_time = time.time()
        late_slice = t.load(data_slice=slice(800, 900))  # Changed from 900-1000 to avoid edge case
        ...
        print(f"Early slice: {early_time:.3f}s, Middle slice: {middle_time:.3f}s, Late slice: {late_time:.3f}s")
    ...
    def test_frequency_resampling_performance(self, large_trajectory_path):
        ...
        for freq in frequencies:
            ...
            print(f"Resampling to {freq}Hz: {resample_time:.3f}s, {actual_len} frames")
    ...
    def test_combined_operations_performance(self, large_trajectory_path):
        ...
        test_cases = [
            {"desired_frequency": 10.0, "data_slice": slice(100, 300)},
            {"desired_frequency": 5.0, "data_slice": slice(0, 500)},
            {"desired_frequency": 2.0, "data_slice": slice(200, 800, 2)},
        ]
        ...
        for i, params in enumerate(test_cases):
            ...
            print(f"Combined operation {i+1}: {operation_time:.3f}s, {result_len} frames")
    ...
    def test_container_return_performance(self, large_trajectory_path):
        ...
        test_cases = [
            {},  # No parameters
            {"data_slice": slice(0, 1000)},  # Large slice
            {"desired_frequency": 1.0},  # Heavy resampling
            {"desired_frequency": 5.0, "data_slice": slice(100, 900)},  # Combined
        ]
        ...
        for feature_count in feature_counts:
            ...
            for i in range(200):  # Fewer timesteps to keep test reasonable
                ...
                for j in range(feature_count):
                    data[f"feature_{j}"] = rng.normal(size=3).astype(np.float32)
    ...
    def test_scaling_with_data_types(self, temp_dir, rng):
        ...
        for i in range(300):
            data = {
                ...
                "large_array": (rng.random((20, 20, 3)) * 255).astype(np.uint8),
            }
            traj.add_by_dict(data, timestamp=i * 100)
        ...
        for i, slice_params in enumerate(test_cases):
            ...
            print(f"Mixed types, slice {i+1}: {load_time:.3f}s, {actual_len} frames")
...
class TestTrajectoryLoaderStressTests:
    ...
    def test_high_frequency_resampling_stress(self, large_trajectory_path):
        ...
        for freq in test_frequencies:
            ...
            print(f"Frequency {freq}Hz: {resample_time:.3f}s, {result_len} frames")
