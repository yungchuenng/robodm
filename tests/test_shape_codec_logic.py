...
class TestShapeBasedCodecSelection:
    ...
    def test_rgb_image_codec_selection(self):
        ...
        assert codec != "rawvideo", f"RGB image with even dimensions should get video codec, got {codec}"
        assert codec in ["libx264", "libx265", "libaom-av1", "ffv1"], f"Got unexpected codec: {codec}"
    ...
    def test_non_rgb_shapes_use_rawvideo(self):
        ...
        for shape, description in test_cases:
            ...
            assert codec == "rawvideo", f"{description} should use rawvideo, got {codec}"
    ...
    def test_user_specified_codec_validation(self):
        ...
        rgb_even = FeatureType(dtype="uint8", shape=(128, 128, 3))
        codec = config.get_codec_for_feature(rgb_even)
        assert codec == "libx264", f"Compatible RGB should use user-specified codec, got {codec}"
        ...
        assert codec == "rawvideo", f"Incompatible RGB should fall back to rawvideo, got {codec}"
...
class TestCodecCompatibilityValidation:
    ...
    def test_is_valid_image_shape(self):
        ...
        test_cases = [
            ...
            ((127, 129, 3), "libx264", False, "Odd dimensions should fail for H.264"),
            ((1920, 1080, 3), "libx264", True, "Large even dimensions should work"),
            ((2, 2, 3), "libx264", True, "Very small even dimensions might work"),
            ((128, 128), "libx264", False, "Non-RGB should not be valid for video codec"),
        ...
        for shape, codec, expected, description in test_cases:
            ...
            assert result == expected, f"{description}: shape {shape} with {codec} expected {expected}, got {result}"

    def test_is_codec_config_supported(self):
        """Test PyAV codec configuration support."""
        # These should work for most systems
        assert CodecConfig.is_codec_config_supported(128, 128, "yuv420p", "libx264")
        
        # Very large dimensions might not work
        large_result = CodecConfig.is_codec_config_supported(10000, 10000, "yuv420p", "libx264")
...
class TestRoundtripData:
    ...
    def test_different_shapes_and_types(self):
        ...
        for shape, dtype, expected_type in test_cases:
            ...
            if expected_type == "video":
                assert codec != "rawvideo", f"Shape {shape} should get video codec, got {codec}"
            else:
                assert codec == "rawvideo", f"Shape {shape} should get rawvideo, got {codec}"
    ...
    def test_mixed_rgb_and_non_rgb_in_trajectory(self):
        ...
        features = {
            "camera/rgb": FeatureType(dtype="uint8", shape=(128, 128, 3)),  # RGB
            "camera/depth": FeatureType(dtype="float32", shape=(128, 128)),  # Depth
            "robot/joint_pos": FeatureType(dtype="float32", shape=(7,)),  # Vector
            "camera/mask": FeatureType(dtype="uint8", shape=(128, 128, 1)),  # Mask
        }
        ...
        assert codecs["camera/depth"] == "rawvideo", "Depth should get rawvideo"
        assert codecs["robot/joint_pos"] == "rawvideo", "Joint positions should get rawvideo"
...
class TestPixelFormatSelection:
    ...
    def test_rgb_pixel_format_selection(self):
        ...
        for codec in yuv_codecs:
            ...
            assert result == "yuv420p", f"RGB data with {codec} should get yuv420p, got {result}"
    ...
    def test_non_rgb_pixel_format_selection(self):
        ...
        for data_type in [grayscale_type, vector_type]:
            for codec in ["libx264", "libx265", "libaom-av1", "ffv1"]:
                ...
                assert result is None, f"Non-RGB data should not get pixel format, got {result}"
    ...
    def test_rawvideo_pixel_format(self):
        ...
        assert result is None, f"rawvideo should return None for pixel format, got {result}"
