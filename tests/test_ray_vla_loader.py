...
from robodm.loader.vla import (
    ...
    create_trajectory_loader, create_slice_loader
)
from robodm.dataset import (
    VLADataset, DatasetConfig,
    load_trajectory_dataset, load_slice_dataset, split_dataset
)


def create_test_trajectory(path: str, num_steps: int = 100, image_size: tuple = (64, 64)):
    ...
    trajectory_data = {
        ...
        "terminated": [
            False if i < num_steps - 1 else True 
...
class TestVLADataset:
    ...
    def test_legacy_compatibility(self, single_trajectory):
        ...
        with pytest.raises(NotImplementedError, match="Random access not supported"):
            _ = dataset[0]
