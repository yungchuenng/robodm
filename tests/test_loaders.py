...
def create_test_trajectory_with_codec(temp_dir, codec, data, filename_suffix=""):
...
class TestNonShuffleVLALoader:
        ...
        for i in range(3):
            ...
            if success:
                ...
                if "not available" in error.lower() or "codec" in error.lower():
                    ...
                    pytest.fail(f"Failed to create trajectory with codec {codec}: {error}")
        
        if not working_paths:
            pytest.skip(f"No trajectories created successfully with codec {codec}")
    ...
    def test_vla_loader_batch_size(self, temp_dir, large_sample_data, codec):
        ...
        path, success, error = create_test_trajectory_with_codec(temp_dir, codec, small_data)
...
class TestVLALoaderCodecValidation:
    ...
    def test_loader_codec_roundtrip_validation(self, temp_dir, codec):
        ...
        test_data = {
            "observation/image": [
                np.random.randint(0, 255, (640, 480, 3), dtype=np.uint8) for _ in range(5)
            ],
            "observation/depth": [
                np.random.random((640, 480)).astype(np.float32) for _ in range(5)
            ],
            "action": [
        ...
        path, success, error = create_test_trajectory_with_codec(temp_dir, codec, test_data)
        ...
        try:
            ...
            pytest.fail(f"VLA loader failed to load data created with codec {codec}: {e}")
    ...
    def test_loader_codec_compatibility_report(self, temp_dir):
        ...
        simple_data = {
            "observation/image": [
                np.random.randint(0, 255, (320, 240, 3), dtype=np.uint8) for _ in range(3)
            ],
            "action": [
                np.array([1.0, 2.0]) for _ in range(3)
            ],
        }
        ...
        for codec in ALL_CODECS:
            try:
                ...
                path, success, error = create_test_trajectory_with_codec(temp_dir, codec, simple_data)
                
                if not success:
                    codec_results[codec] = {"status": "failed_creation", "error": error}
                    continue
                ...
                if len(trajectories) == 1 and isinstance(trajectories[0], dict):
                    codec_results[codec] = {"status": "success", "error": None}
                else:
                    codec_results[codec] = {"status": "failed_loading", "error": "Invalid trajectory data"}
                    
            except Exception as e:
                codec_results[codec] = {"status": "failed_exception", "error": str(e)}
        ...
        assert len(successful_codecs) > 0, "No codecs are compatible with VLA loader!"
    ...
    def test_loader_with_problematic_data(self, temp_dir, codec):
        ...
        edge_cases = [
            {
                "name": "small_images",
                "data": {
                    "observation/image": [np.random.randint(0, 255, (128, 128, 3), dtype=np.uint8) for _ in range(2)],
            ...
            },
            {
                ...
                "data": {
                    "observation/image": [np.random.randint(0, 255, (320, 240, 3), dtype=np.uint8)],
            ...
            },
            {
                ...
                "data": {
                    ...
                    "action": [np.array([0.0]), np.array([1.0]), np.array([0.5])],
                }
            },
        ]
        ...
        for case in edge_cases:
            try:
                path, success, error = create_test_trajectory_with_codec(
                    temp_dir, codec, case["data"], filename_suffix=f"_{case['name']}"
                )
                ...
                if not success:
                    if "not available" in error.lower():
                        ...
                        print(f"Codec {codec} failed with {case['name']} (may be expected): {error}")
                        continue
                ...
                if "not available" in error_msg.lower():
                    ...
                    pytest.fail(f"Codec {codec} has encoding/decoding issues with {case['name']}: {error_msg}")
                else:
                    # Some failures might be expected for edge cases
                    print(f"Codec {codec} failed with {case['name']} (may be expected): {error_msg}")
    ...
    def test_vla_vs_hdf5_data_consistency(self, temp_dir, sample_dict_of_lists, benchmark_dataset):
        ...
        for key in vla_data.keys():
            ...
            if vla_array.dtype == np.uint8:
                ...
                assert max_diff <= 2.0, f"Max pixel difference {max_diff} too large between VLA and HDF5"
                assert mean_diff <= 0.5, f"Mean pixel difference {mean_diff} too large between VLA and HDF5"
            else:
                # Float data should be nearly identical for lossless
                np.testing.assert_allclose(vla_array, h5_array, rtol=1e-5, atol=1e-6)
