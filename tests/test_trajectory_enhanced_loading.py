...
def _make_step(rng: np.random.Generator, idx: int) -> Dict[str, object]:
    ...
    return {
        ...
        "camera_rgb": (rng.random((64, 64, 3)) * 255).astype(np.uint8),  # RGB image
        "depth_map": rng.random((32, 32)).astype(np.float32),     # depth/float32
        "metadata": {"step": idx, "tag": f"step_{idx}"},          # nested dict
    }
...
def trajectory_path(temp_dir, base_trajectory_data) -> str:
    ...
    for i, step_data in enumerate(base_trajectory_data):
        ...
        data_without_timestamp = {k: v for k, v in step_data.items() if k != "timestamp"}
...
class TestTrajectoryLoad:
    ...
    def test_load_returns_correct_keys(self, trajectory_path):
        ...
        expected_keys = {
            "robot_position", "joint_angles", "action", "gripper_state", 
            "sensor_reading", "camera_rgb", "depth_map", "metadata/step", "metadata/tag"
        }
    ...
    def test_slice_invalid_negative(self, trajectory_path):
        t = Trajectory(trajectory_path, mode="r")
        with pytest.raises(ValueError, match="Negative slice start values are not supported"):
    ...
    def test_slice_invalid_step(self, trajectory_path):
        ...
        # Zero step
        with pytest.raises(ValueError, match="Reverse or zero-step slices are not supported"):
    ...
    @pytest.mark.parametrize("freq, expect_factor", [(5.0, 0.5), (2.0, 0.2), (1.0, 0.1)])
    ...
    def test_resampling_frequency_edge_cases(self, trajectory_path):
        ...
        assert all(len(v) <= 2 for v in very_low.values())  # At most 1-2 frames
    ...
    def test_resampling_invalid_frequency(self, trajectory_path):
        ...
        # Zero frequency
        with pytest.raises(ValueError, match="desired_frequency must be positive"):
    ...
    def test_invalid_return_type(self, trajectory_path):
        ...
        t = Trajectory(trajectory_path, mode="r")
        with pytest.raises(ValueError, match="return_type must be 'numpy' or 'container'"):
    ...
    def test_different_codecs_roundtrip(self, temp_dir, base_trajectory_data, codec):
    ...
    def test_complex_feature_names(self, temp_dir, rng):
        ...
        nested_data = {
            "robot": {
                "arm": {"joint_0": 1.0, "joint_1": 2.0},
                "base": {"x": 0.0, "y": 1.0}
            },
            "sensor": {
                "camera": {"rgb": rng.random((8, 8, 3)), "depth": rng.random((8, 8))}
            }
        }
        ...
        expected_keys = {
            "robot/arm/joint_0", "robot/arm/joint_1", 
            "robot/base/x", "robot/base/y",
...
class TestTrajectoryLoadIntegration:
    ...
    def test_full_pipeline_integration(self, temp_dir, rng):
        ...
        for i in range(50):
            ...
            traj.add_by_dict(step_data, timestamp=int(i * 20))  # 20ms intervals
    ...
    def test_robustness_with_malformed_data(self, temp_dir):
        ...
        for i in range(10):
            traj.add_by_dict({"value": i, "data": np.array([i, i+1])}, timestamp=i * 100)
